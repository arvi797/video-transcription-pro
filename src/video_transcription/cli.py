...
Examples:
  ...
  video-transcribe video.mp4 -f txt json srt
    ...
    parser.add_argument(
        "input",
        help="Input video file or folder (for batch processing)"
    )
    ...
    parser.add_argument(
        "--speaker-names",
        help="JSON file with speaker name mappings"
    )
    ...
    parser.add_argument(
        "--batch",
        action="store_true",
        help="Process entire folder (batch mode)"
    )
    ...
    parser.add_argument(
        "--auth-token",
        help="HuggingFace authentication token for PyAnnote"
    )
