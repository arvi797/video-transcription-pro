...
class BatchProcessor:
    ...
    def process_folder(self,
        ...
        print(f"🔄 Processing mode: {'Parallel' if parallel and self.max_workers > 1 else 'Sequential'}")
        print("=" * 60)
    ...
    def _find_media_files(self,
                          folder: Path,
                          recursive: bool = False,
                          filter_extensions: Optional[List[str]] = None) -> List[Path]:
        """Find all media files in a folder."""
        extensions_to_use = set(filter_extensions) if filter_extensions else self.supported_extensions
    ...
    def _process_sequential(self, file_paths: List[Path], output_folder: Path) -> List[Dict]:
    ...
    def _process_parallel(self, file_paths: List[Path], output_folder: Path) -> List[Dict]:
    ...
    def _process_single_file(self, file_path: Path, output_dir: Path, file_index: int, total_files: int) -> Dict:
    ...
    def _save_batch_summary(self, results: List[Dict], output_folder: Path, processing_time: float) -> None:
        ...
        summary_path = output_folder / f"batch_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
