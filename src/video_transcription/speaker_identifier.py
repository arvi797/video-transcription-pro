...
class SpeakerIdentifier:
    ...
    def __init__(self, auth_token: Optional[str] = None):
        ...
        for method, available in self.methods_available.items():
            ...
            device_info = " (GPU)" if method == 'pyannote' and available and torch.cuda.is_available() else ""
    ...
    def _check_pyannote(self) -> bool:
        ...
        try:
            ...
            if hasattr(self, 'pyannote_pipeline') and self.pyannote_pipeline is not None:
                return True
            ...
            print("⚠️ PyAnnote Audio not installed. Install with: pip install pyannote.audio")
            return False
        except Exception as e:
            print(f"⚠️ PyAnnote setup issue: {e}")
            print("💡 You may need to accept model licenses at https://huggingface.co/pyannote/speaker-diarization-3.1")
            return False
    ...
    def _pyannote_diarization(self, 
                             audio_path: Union[str, Path], 
                             whisper_result: Dict, 
        ...
        for segment in segments:
            ...
            for turn, _, speaker in diarization.itertracks(yield_label=True):
                if (turn.start <= start_time_seg < turn.end or 
                    turn.start < end_time_seg <= turn.end or
                    (start_time_seg <= turn.start and end_time_seg >= turn.end)):
        ...
        return {
            ...
            'accuracy_score': 95,
            'confidence': 'very_high',
            'num_speakers': len(set(seg['speaker'] for seg in speaker_segments if seg['speaker'] != 'Speaker_Unknown')),
            'processing_time': diarization_time,
            'device': device_name
        }
    
    def _audio_clustering(self, 
                         audio_path: Union[str, Path], 
                         whisper_result: Dict, 
                         num_speakers: Optional[int] = None) -> Dict:
        """
        ...
        speaker_segments = []
        speakers = ["Speaker_A", "Speaker_B", "Speaker_C", "Speaker_D", "Speaker_E", "Speaker_F"]
    ...
    def get_info(self) -> Dict:
        ...
        return {
            ...
            'pyannote_device': str(self.pyannote_device) if self.pyannote_device else None,
