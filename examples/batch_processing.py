    ...
    if successful > 0:
        ...
        for result in results:
            if result.get("success"):
                ...
                print(
                    f"   📹 {video_name}: {accuracy}% accuracy, {speakers} speakers, {duration:.1f}min"
                )
...
def monitor_large_batch():
    ...
    print(
        f"   GPU available: {info['pipeline']['components']['transcriber']['gpu_available']}"
    )
    ...
    if Path(large_folder).exists():
        ...
        print(
            f"   Average accuracy: {sum(r['summary']['accuracy'] for r in results if r.get('success')) / len([r for r in results if r.get('success')]):.1f}%"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"📊 Total files processed across all examples: {len(main_results) + len(specific_results) + len(filtered_results)}"
    )
